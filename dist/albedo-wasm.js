...
export class Bucket {
    ...
    insert(data) {
        ...
        const dataBytes = dataBuf instanceof Uint8Array ? dataBuf : new Uint8Array(dataBuf);
        const dataPtr = this.exports.albedo_malloc(dataBytes.length);
        new Uint8Array(this.exports.memory.buffer, dataPtr, dataBytes.length).set(dataBytes);
        try {
    ...
    ensureIndex(field, options = Bucket.defaultIndexOptions) {
        ...
        try {
            const res = exports.albedo_ensure_index(this.pointer, pathAlloc.ptr, optionFlags);
    ...
    delete(query, _options = {}) {
        ...
        const queryBytes = queryBuf instanceof Uint8Array ? queryBuf : new Uint8Array(queryBuf);
    ...
    *list(query = {}, options = {}) {
        ...
        if (options.sort)
            finalQuery.sort = options.sort;
        if (options.sector)
            finalQuery.sector = options.sector;
        if (options.projection)
            finalQuery.projection = options.projection;
        ...
        try {
            while (true) {
                ...
                const shouldQuit = yield deserialize(new Uint8Array(this.memory.buffer, docPtr, docLen));
                if (shouldQuit) {
                    break;
                }
            }
    ...
    *transformCursor(query = {}) {
        ...
        try {
            while (true) {
                ...
                const docLen = this.memory.getInt32(docPtr, true);
                const currentDoc = deserialize(new Uint8Array(this.memory.buffer, docPtr, docLen));
                ...
                if (transformDoc === undefined) {
                    ...
                    const transformBytes = transformBuf instanceof Uint8Array
                        ? transformBuf
                        : new Uint8Array(transformBuf);
                    transformLen = transformBytes.length;
                    transformPtr = exports.albedo_malloc(transformLen);
                    new Uint8Array(this.memory.buffer, transformPtr, transformLen).set(transformBytes);
                }
