...
export class ObjectId {
    constructor(input) {
        if (typeof input === "string") {
            ...
            const arr = new Uint8Array(12);
            const src = input;
            for (let i = 0; i < Math.min(12, src.length); i++)
                arr[i] = src[i];
            this.id = arr;
        }
    }
    static fromString(hex) {
        if (hex.length !== 24)
            throw new Error("ObjectId hex must be 24 hex chars");
        const arr = new Uint8Array(12);
        for (let i = 0; i < 12; i++) {
...
export function defaultInspect(x, _options) {
...
function encodeElement(writer, key, value) {
    if (value instanceof ObjectId) {
        writer.writeUint8(7 /* Type.ObjectId */);
    ...
    if (typeof value === "number") {
        if (Number.isInteger(value) &&
            value <= 0x7fffffff &&
            value >= -0x80000000) {
            writer.writeUint8(16 /* Type.Int32 */);
            writer.writeCString(key);
            writer.writeInt32(value | 0);
        }
        else {
            writer.writeUint8(1 /* Type.Double */);
            writer.writeCString(key);
            writer.writeFloat64(value);
        }
    }
    else if (typeof value === "bigint") {
        writer.writeUint8(18 /* Type.Int64 */);
        writer.writeCString(key);
        writer.writeBigInt64(value);
    }
    else if (typeof value === "string") {
        writer.writeUint8(2 /* Type.String */);
        writer.writeCString(key);
        writer.writeString(value);
    }
    else if (value === null || value === undefined) {
        writer.writeUint8(10 /* Type.Null */);
        writer.writeCString(key);
    }
    else if (typeof value === "boolean") {
        writer.writeUint8(8 /* Type.Boolean */);
        writer.writeCString(key);
        writer.writeUint8(value ? 1 : 0);
    }
    else if (value instanceof Date) {
        writer.writeUint8(9 /* Type.DateTime */);
        writer.writeCString(key);
        ...
        writer.writeUint8(5 /* Type.Binary */);
        writer.writeCString(key);
        ...
        writer.writeUint8(11 /* Type.Regex */);
        writer.writeCString(key);
        ...
        writer.writeUint8(4 /* Type.Array */);
        writer.writeCString(key);
        writeArray(writer, value);
    }
    else if (isPlainObject(value)) {
        writer.writeUint8(3 /* Type.Document */);
        writer.writeCString(key);
...
function writeDocument(writer, obj) {
...
function writeArray(writer, arr) {
    const start = writer.beginContainer();
...
function readInt64AsBigInt(buf, offset) {
...
function decodeDocument(buf, offset = 0) {
